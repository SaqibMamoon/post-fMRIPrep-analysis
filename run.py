...
def main():
    ...
    for participant in opts.participant_label:
      if opts.participant_label:
          query['subject'] = participant
      if opts.run:
      ...
      if 'participant' in opts.analysis_level:
          ...
          for part in prepped_bold:
              ...
              inputs[sub]['events'] = layout.get(suffix='events', return_type='file',**subquery)[0]
              inputs[sub]['regressors'] = layout.get(domains='derivatives',suffix='regressors',return_type='file',extensions=['.tsv'],**subquery)[0]
